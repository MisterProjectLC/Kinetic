...
void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float ColorSensitivity, float4 OutlineColor, out float4 Out)
{
    ...
    // Black Abyss
    float antiEdgeAbyss = (sqrt(dot(colorSamples[0], colorSamples[0])) + sqrt(dot(colorSamples[1], colorSamples[1])) + sqrt(dot(colorSamples[2], colorSamples[2])) + sqrt(dot(colorSamples[3], colorSamples[3])) < 0.015) ? 0 : 1;

    // Edge intensity is the biggest value between depth, normals and color difference 
    float edge = min(max(edgeDepth, max(edgeNormal, edgeColor)), antiEdgeAbyss);
